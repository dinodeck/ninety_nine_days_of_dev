...
function ShopState:RenderStock(menu, renderer, x, y, item)
    ...
    font:DrawText2d(renderer, self.mPriceX, y, ": ")
    font:AlignTextX("right")
...
function ShopState:RenderInventory(menu, renderer, x, y, item)
    ...
    local def = ItemDB[item.id]
    font:AlignTextX("left")
    font:DrawText2d(renderer, self.mPriceX - 18, y, ":", color)
...
function ShopState:DrawCharacters(renderer, itemDef)
    ...
    for _, actor in ipairs(self.mActors) do
        ...
        if equipped then
            ...
            font:DrawText2d(renderer, equipX + 2, equipY - 2, "E", shadowColor)
            font:DrawText2d(renderer, equipX, equipY, "E")
        end
...
function ShopState:Render(renderer)
    ...
    font:DrawText2d(renderer, tX, tY, self.mName)
    ...
    elseif self.mState == "buy" then
        ...
        font:AlignText("center", "center")
        font:DrawText2d(renderer, x, y, buyMessage)
        font:AlignText("left", "center")
        self.mStock:Render(renderer)
    ...
    elseif self.mState == "sell" then
        ...
        font:AlignText("center", "center")
        font:DrawText2d(renderer, x, y, sellMessage)
        font:AlignText("left", "center")
        self.mInventory:Render(renderer)
    ...
    font:DrawText2d(renderer, statusX, statusY, gpText)
    font:DrawText2d(renderer, statusX, statusY - 32, invText)
    font:DrawText2d(renderer, statusX, statusY - 50, equipText)

    font:AlignText("left", "center")
    font:DrawText2d(renderer, valueX, statusY, gpValue)
    font:DrawText2d(renderer, valueX, statusY - 32, invValue)
    font:DrawText2d(renderer, valueX, statusY - 50, equipValue)

    font:AlignText("left", "center")
    -- renderer:ScaleText(1,1)
    local descX = self.mLayout:Left("desc") + 8
    local descY = self.mLayout:MidY("desc")
    font:DrawText2d(renderer, descX, descY, self.mItemDescription)
