...
function CombatState:Create(stack, def)
    ...
    {
        {
            text = 'N', -- Links to "NAME" in the def
            x = x + 32,
            y = y
        },
        {
            text = 'H', -- Links to "HP" in the def
            x = layout:Left('right') + 24,
            y = y
        },
        {
            text = 'M', -- Links to "M" in the def
...
function CombatState:RenderPartyNames(renderer, x, y, item)
    ...
    font:DrawText2d(renderer, x, y, item.mName, nameColor)
end

function CombatState:DrawHP(renderer, x, y, hp, max)

    local font = gGame.Font.stat
...
function CombatState:RenderPartyStats(renderer, x, y, item)
    ...
    self:DrawHP(renderer, x, y, stats:Get('hp_now'), stats:Get('hp_max'))
...
function CombatState:Render(renderer)
    ...
    if self.mShowTip then
        ...
        font:DrawText2d(renderer, x, y, self.mTipText)
    end
    ...
    if self.mShowNotice then
        ...
        font:DrawText2d(renderer, x, y, self.mNoticeText)
    end
    ...
    for k, v in ipairs(self.mPanelTitles) do
        local font = gGame.Font.stat
        font:DrawText2d(renderer, v.x, v.y, v.text, self.mTitleColor)
    end
