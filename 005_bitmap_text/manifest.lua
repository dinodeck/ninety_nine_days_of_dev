--
-- A manifest of all the game's assets
--
manifest =
{
    scripts =
    {
        ['Actions.lua'] =
        {
            path = "code/Actions.lua"
        },
        ['Character.lua'] =
        {
            path = "code/Character.lua"
        },
        ['CaptionState.lua'] =
        {
            path = "code/CaptionState.lua"
        },
        ['CaptionStyles.lua'] =
        {
            path = "code/CaptionStyles.lua"
        },
        ['Dependencies.lua'] =
        {
            path = "code/Dependencies.lua"
        },
        ['Entity.lua'] =
        {
            path = "code/Entity.lua"
        },
        ['EntityDefs.lua'] =
        {
            path = "code/EntityDefs.lua"
        },
        ['ExploreState.lua'] =
        {
            path = "code/ExploreState.lua"
        },
        ['FrontMenuState.lua'] =
        {
            path = "code/FrontMenuState.lua"
        },
        ['FollowPathState.lua'] =
        {
            path = "code/FollowPathState.lua"
        },
        ['GameOverState.lua'] =
        {
            path = "code/GameOverState.lua"
        },
        ['ItemMenuState.lua'] =
        {
            path = "code/ItemMenuState.lua"
        },
        ['ItemDB.lua'] =
        {
            path = "code/ItemDB.lua"
        },
        ['InGameMenuState.lua'] =
        {
            path = "code/InGameMenuState.lua"
        },
        ['Icons.lua'] =
        {
            path = "code/Icons.lua"
        },
        ['Layout.lua'] =
        {
            path = "code/Layout.lua"
        },
        ['main.lua'] =
        {
            path = "code/main.lua"
        },
        ['Map.lua'] =
        {
            path = "code/Map.lua"
        },
        ['MapDB.lua'] =
        {
            path = "code/MapDB.lua"
        },
        ['MoveState.lua'] =
        {
            path = "code/MoveState.lua"
        },
        ['NPCStandState.lua'] =
        {
            path = "code/NPCStandState.lua"
        },
        ['Panel.lua'] =
        {
            path = "code/Panel.lua"
        },
        ['PlanStrollState.lua'] =
        {
            path = "code/PlanStrollState.lua"
        },
        ['ProgressBar.lua'] =
        {
            path = "code/ProgressBar.lua"
        },
        ['ScreenState.lua'] =
        {
            path = "code/ScreenState.lua"
        },
        ['Scrollbar.lua'] =
        {
            path = "code/Scrollbar.lua"
        },
        ['Selection.lua'] =
        {
            path = "code/Selection.lua"
        },
        ['StateMachine.lua'] =
        {
            path = "code/StateMachine.lua"
        },
        ['StateStack.lua'] =
        {
            path = "code/StateStack.lua"
        },
        ['Storyboard.lua'] =
        {
            path = "code/Storyboard.lua"
        },
        ['StoryboardEvents.lua'] =
        {
            path = "code/StoryboardEvents.lua"
        },
        ['Textbox.lua'] =
        {
            path = "code/Textbox.lua"
        },
        ['TitleScreenState.lua'] =
        {
            path = "code/TitleScreenState.lua"
        },
        ['Trigger.lua'] =
        {
            path = "code/Trigger.lua"
        },
        ['Tween.lua'] =
        {
            path = "code/Tween.lua"
        },
        ['Animation.lua'] =
        {
            path = "code/Animation.lua"
        },
        ['Util.lua'] =
        {
            path = "code/Util.lua"
        },
        ['WaitState.lua'] =
        {
            path = "code/WaitState.lua"
        },
        ['World.lua'] =
        {
            path = "code/World.lua"
        },
        ['Stats.lua'] =
        {
            path = "code/Stats.lua"
        },
        ['StatDefs.lua'] =
        {
            path = "code/StatDefs.lua"
        },
        ['Dice.lua'] =
        {
            path = "code/Dice.lua"
        },
        ['PartyMemberDefs.lua'] =
        {
            path = "code/PartyMemberDefs.lua"
        },
        ['Party.lua'] =
        {
            path = "code/Party.lua"
        },
        ['Actor.lua'] =
        {
            path = "code/Actor.lua"
        },
        ['LevelFunction.lua'] =
        {
            path = "code/LevelFunction.lua"
        },
        ['ActorSummary.lua'] =
        {
            path = "code/ActorSummary.lua"
        },
        ['ActorXPSummary.lua'] =
        {
            path = "code/ActorXPSummary.lua"
        },
        ['StatusMenuState.lua'] =
        {
            path = "code/StatusMenuState.lua"
        },
        ['EquipMenuState.lua'] =
        {
            path = "code/EquipMenuState.lua"
        },
        ['CombatState.lua'] =
        {
            path = "code/CombatState.lua"
        },
        ['EnemyDefs.lua'] =
        {
            path = "code/EnemyDefs.lua"
        },
        ['CSRunAnim.lua'] =
        {
            path = "code/combat_states/CSRunAnim.lua"
        },
        ['CSHurt.lua'] =
        {
            path = "code/combat_states/CSHurt.lua"
        },
        ['CSMove.lua'] =
        {
            path = "code/combat_states/CSMove.lua"
        },
        ['CSStandby.lua'] =
        {
            path = "code/combat_states/CSStandby.lua"
        },
        ['CSEnemyHurt.lua'] =
        {
            path = "code/combat_states/CSEnemyHurt.lua"
        },
        ['CSEnemyDie.lua'] =
        {
            path = "code/combat_states/CSEnemyDie.lua"
        },
        ['Animation.lua'] =
        {
            path = "code/Animation.lua"
        },
        ['EventQueue.lua'] =
        {
            path = "code/EventQueue.lua"
        },
        ['CombatTargetState.lua'] =
        {
            path = "code/CombatTargetState.lua"
        },
        ['CETurn.lua'] =
        {
            path = "code/combat_events/CETurn.lua"
        },
        ['CEAttack.lua'] =
        {
            path = "code/combat_events/CEAttack.lua"
        },
        ['CEFlee.lua'] =
        {
            path = "code/combat_events/CEFlee.lua"
        },
        ['CEUseItem.lua'] =
        {
            path = "code/combat_events/CEUseItem.lua"
        },
        ['CECastSpell.lua'] =
        {
            path = "code/combat_events/CECastSpell.lua"
        },
        ['CESlash.lua'] =
        {
            path = "code/combat_events/CESlash.lua"
        },
        ['CESteal.lua'] =
        {
            path = "code/combat_events/CESteal.lua"
        },
        ['CombatChoiceState.lua'] =
        {
            path = "code/CombatChoiceState.lua"
        },
        ['CombatFormula.lua'] =
        {
            path = "code/CombatFormula.lua"
        },
        ['JumpingNumbers.lua'] =
        {
            path = "code/fx/JumpingNumbers.lua"
        },
        ['CombatTextFx.lua'] =
        {
            path = "code/fx/CombatTextFx.lua"
        },
        ['AnimEntityFx.lua'] =
        {
            path = "code/fx/AnimEntityFx.lua"
        },
        ['XPSummaryState.lua'] =
        {
            path = "code/XPSummaryState.lua"
        },
        ['LootSummaryState.lua'] =
        {
            path = "code/LootSummaryState.lua"
        },
        ['XPPopUp.lua'] =
        {
            path = "code/XPPopUp.lua"
        },
        ['OddmentTable.lua'] =
        {
            path = "code/OddmentTable.lua"
        },
        ['BrowseListState.lua'] =
        {
            path = "code/BrowseListState.lua"
        },
        ['CombatActions.lua'] =
        {
            path = "code/CombatActions.lua"
        },
        ['SpellDB.lua'] =
        {
            path = "code/SpellDB.lua"
        },
        ['SpecialDB.lua'] =
        {
            path = "code/SpecialDB.lua"
        },
        ['ArenaState.lua'] =
        {
            path = "code/ArenaState.lua"
        },
        ['ArenaCompleteState.lua'] =
        {
            path = "code/ArenaCompleteState.lua"
        },
        ['map_town.lua'] =
        {
            path = "map/map_town.lua"
        },
        ['map_world.lua'] =
        {
            path = "map/map_world.lua"
        },
        ['ShopState.lua'] =
        {
            path = "code/ShopState.lua"
        },
        ['map_cave.lua'] =
        {
            path = "map/map_cave.lua"
        },
        ['DefaultGameState.lua'] =
        {
            path = "code/DefaultGameState.lua"
        },
        ['SaveScheme.lua'] =
        {
            path = "code/SaveScheme.lua"
        },
        ['PrintTable.lua'] =
        {
            path = "code/PrintTable.lua"
        },
        ['Blob.lua'] =
        {
            path = "code/Blob.lua"
        },
        ['Save.lua'] =
        {
            path = "code/Save.lua"
        },
        ['BitmapText.lua'] =
        {
            path = "code/BitmapText.lua"
        },
        ['NumberFontDef.lua'] =
        {
            path = "code/font/NumberFontDef.lua"
        },
        ['DamageFontDef.lua'] =
        {
            path = "code/font/DamageFontDef.lua"
        },
        ['DamageSpriteDef.lua'] =
        {
            path = "code/font/DamageSpriteDef.lua"
        },
        ['CombatSpriteFx.lua'] =
        {
            path = "code/fx/CombatSpriteFx.lua"
        },
        ['StatsFontDef.lua'] =
        {
            path = "code/font/StatsFontDef.lua"
        },
        ['DefaultFontDef.lua'] =
        {
            path = "code/font/DefaultFontDef.lua"
        },
    },
    textures =
    {
        ['walk_cycle.png'] =
        {
            path = "art/walk_cycle.png",
        },
        ['combat_hero.png'] =
        {
            path = "art/combat_hero.png",
        },
        ['combat_thief.png'] =
        {
            path = "art/combat_thief.png",
        },
        ['combat_mage.png'] =
        {
            path = "art/combat_mage.png",
        },
        ['goblin.png'] =
        {
            path = "art/goblin.png",
        },
        ['cursor.png'] =
        {
            path = "art/cursor.png",
            scale = "pixelart"
        },
        ['gradient_panel.png'] =
        {
            path = "art/gradient_panel.png",
        },
        ['continue_caret.png'] =
        {
            path = "art/continue_caret.png",
        },
        ['scrollbar.png'] =
        {
            path = "art/scrollbar.png",
            scale = "pixelart"
        },
        ['inventory_icons.png'] =
        {
            path = "art/inventory_icons.png",
        },
        ['mage_portrait.png'] =
        {
            path = "art/mage_portrait.png",
            scale = "pixelart"
        },
        ['thief_portrait.png'] =
        {
            path = "art/thief_portrait.png",
            scale = "pixelart"
        },
        ['hero_portrait.png'] =
        {
            path = "art/hero_portrait.png",
            scale = "pixelart"
        },
        ['xpbackground.png'] =
        {
            path = "art/xpbackground.png",
        },
        ['xpforeground.png'] =
        {
            path = "art/xpforeground.png",
        },
        ['hpbackground.png'] =
        {
            path = "art/hpbackground.png",
        },
        ['hpforeground.png'] =
        {
            path = "art/hpforeground.png",
        },
        ['mpbackground.png'] =
        {
            path = "art/mpbackground.png",
        },
        ['mpforeground.png'] =
        {
            path = "art/mpforeground.png",
        },
        ['combat_slash.png'] =
        {
            path = "art/combat_slash.png",
        },
        ['combat_claw.png'] =
        {
            path = "art/combat_claw.png",
        },
        ['fx_restore_hp.png'] =
        {
            path = "art/fx_restore_hp.png",
        },
        ['fx_use_item.png'] =
        {
            path = "art/fx_use_item.png",
        },
        ['fx_restore_mp.png'] =
        {
            path = "art/fx_restore_mp.png",
        },
        ['fx_revive.png'] =
        {
            path = "art/fx_revive.png",
        },
        ['fx_electric.png'] =
        {
            path = "art/fx_electric.png",
        },
        ['fx_ice.png'] =
        {
            path = "art/fx_ice.png",
        },
        ['fx_fire.png'] =
        {
            path = "art/fx_fire.png",
        },
        ['town_tileset.png'] =
        {
            path = "art/town_tileset.png"
        },
        ['world_tileset.png'] =
        {
            path = "art/world_tileset.png"
        },
        ['up_caret.png'] =
        {
            path = "art/up_caret.png"
        },
        ['combat_bg_field.png'] =
        {
            path = "art/combat_bg_field.png"
        },
        ['combat_bg_forest.png'] =
        {
            path = "art/combat_bg_forest.png"
        },
        ['goblin_field.png'] =
        {
            path = "art/goblin_field.png",
            scale = "pixelart",
        },
        ['goblin_forest.png'] =
        {
            path = "art/goblin_forest.png",
            scale = "pixelart",
        },
        ['cave_tileset.png'] =
        {
            path = "art/cave_tileset.png"
        },
        ['cave_drake.png'] =
        {
            path = "art/cave_drake.png",
            scale = "pixelart"
        },
        ['cave_shade.png'] =
        {
            path = "art/cave_shade.png",
            scale = "pixelart"
        },
        ['cave_bat.png'] =
        {
            path = "art/cave_bat.png"
        },
        ['combat_bg_cave.png'] =
        {
            path = "art/combat_bg_cave.png"
        },
        ['chest.png'] =
        {
            path = "art/chest.png"
        },
        ['teleport.png'] =
        {
            path = "art/teleport.png"
        },
        ['door.png'] =
        {
            path = "art/door.png"
        },
        ['sphere.png'] =
        {
            path = "art/sphere.png"
        },
        ['title_screen.png'] =
        {
            path = "art/title_screen.png"
        },
        ['save_point.png'] =
        {
            path = "art/save_point.png"
        },
        ['combat_bg_town.png'] =
        {
            path = "art/combat_bg_town.png"
        },
        ['demon.png'] =
        {
            path = "art/demon.png"
        },
        ['number_font.png'] =
        {
            path = "art/font/number_font.png",
            scale = "pixelart"
        },
        ['stats_font.png'] =
        {
            path = "art/font/stats_font.png",
            scale = "pixelart"
        },
        ['damage_font.png'] =
        {
            path = "art/font/damage_font.png",
            scale = "pixelart"
        },
        ['default_font.png'] =
        {
            path = "art/font/default_font.png",
            scale = "pixelart"
        },
    },
    ['fonts'] =
    {
        ["default"] =
        {
            path = "art/junction.ttf",
        },
    },
    ['sounds'] =
    {
    }
}